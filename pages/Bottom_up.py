...
def stakeholder_punkte():
    ...
    with st.sidebar:
        st.markdown("---")
        thema = st.selectbox('Thema auswählen', options=['Klimawandel'], index=0, key='stakeholder_thema')
        unterthema = st.selectbox('Unterthema auswählen', options=['Anpassung an den Klimawandel', 'Klimaschutz', 'Energie'], index=0, key='stakeholder_unterthema')
        unter_unterthema = st.text_input('Unter-Unterthema eingeben', key='stakeholder_unter_unterthema')
        add_row = st.button('Hinzufügen', key='stakeholder_add_row')
    ...
    grid_response = AgGrid(
        ...
        return_mode=DataReturnMode.__members__['AS_INPUT'],  # Adjust the DataReturnMode as per available options
        selection_mode='multiple',
        key='stakeholder_aggrid'
    )

    add_empty_row = st.button('Leere Zeile hinzufügen', key='stakeholder_add_empty_row')
    if add_empty_row:
        empty_row = {"Thema": "", "Unterthema": "", "Unter-Unterthema": ""}
        st.session_state.df3 = st.session_state.df3._append(empty_row, ignore_index=True)
        st.experimental_rerun()

    delete_rows = st.button('Ausgewählte Zeilen löschen', key='stakeholder_delete_rows')
    if delete_rows:
        selected_rows = grid_response['selected_rows']
        selected_indices = [row['index'] for row in selected_rows]
    ...
    save_changes = st.button('Änderungen speichern', key='stakeholder_save_changes')
    if save_changes:
